...
- Bounded recent-driver fallback: if a complete zeroed GPU sample temporarily removes the instantaneous memory/power signal, the manager can rely on a recent multi-sample utilization window, label that basis explicitly, and let it expire quickly when the worker really stops.
...
- Public worker and job records expose the current attempt plus restart count and bounded restart budget, making recovery history visible without publishing PIDs or command lines.
...
- Bounded task handlers can publish a short redacted result note alongside their aggregate metrics; the public task ledger renders allowlisted counts such as sources, words, tests, and checks so Research and Operations show what completed without exposing prompts, credentials, paths, or unrestricted model output.
- The runner can opt into bounded queue observation with `queue_dispatch.enabled`; unhandled work is recorded and deferred until a real handler is connected, preserving the task instead of inventing completion.
- Background queue dispatch keeps model-backed or network-backed handlers off the protected worker's health-sampling loop, with a bounded in-flight slot count and requeue-on-shutdown behavior.
- Optional recurring task templates schedule deterministic, non-overlapping research or build tasks through SQLite, recover the same queued task after a restart, and perform one bounded catch-up instead of creating a burst.
- A disabled-by-default Superteam Earn discovery worker can query agent-eligible listings after a locally stored agent credential is configured; it records safe listing summaries without submitting or claiming work.
- A bounded verification worker can run the repository test suite as durable recurring work, retain a local evidence artifact, and publish only test count, pass state, and duration.
...
- Public agent views show whether a specialist is currently working, its bounded run timing, durable run record, completed task count, and next run without publishing prompts or unrestricted reasoning. The AI run timeline ties each visible start and decision back to that record and its assigned focus.
...
The operations view separates queued work, agent reviews currently running, completed task records, and a recent sanitized task ledger, so a quiet worker interval is distinguishable from work that has never been scheduled. The public ledger interleaves recent records from each task kind within its compact 32-row window, so frequent agent reviews cannot hide research or verification work. The Research view also shows completed research task lifecycle events and aggregate source metrics when that lane has run.
