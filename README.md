...
The operations view separates queued work, agent reviews currently running, completed task records, and a recent sanitized task ledger, so a quiet worker interval is distinguishable from work that has never been scheduled. The Research view also shows completed research task lifecycle events and aggregate source metrics when that lane has run.
